...
// pointmass[0] is the mass, the next dim values are dimension coordinates, and the last ones are velocities
...
void genBodies(int numPoints, typename Kokkos::View<pointmass_arr>::HostMirror &bodies,
  ...
  if(fixedSeed) {
	...
	std::uniform_real_distribution<real> genMass(1.52587890625e-5,
...
void computeForce(const int numPoints, const int curPoint, const Kokkos::View<pointmass_arr> &bodies, force &rF) {
	/* G in kg m^3 / s^2 */
	...
	Kokkos::parallel_reduce<>(numPoints - 1, KOKKOS_LAMBDA (const int i, force &f) {
			int idx;
			if(i >= curPoint) {
				idx = i + 1;
			}
...
void NBodySim(int numPoints, real timestep, real maxTime) {
	Kokkos::View<pointmass_arr> bodies("Point Masses", numPoints);
	typename Kokkos::View<pointmass_arr>::HostMirror
		h_bodies = Kokkos::create_mirror_view(bodies);
	Kokkos::View<pointmass_arr> nextBodies("Point Masses", numPoints);
	genBodies(numPoints, h_bodies);
	for(real curTime = 0.0; curTime < maxTime; curTime += timestep) {
		Kokkos::parallel_for(numPoints, KOKKOS_LAMBDA (const int i) {
				force f;
				computeForce(numPoints, i, bodies, f);
				for(int j = 1; j <= dim; j++) {
					nextBodies(i, j) += nextBodies(i, j + dim) * timestep;
...
int main(int argc, char **argv) {
	...
	t.stopTimer();
	std::cout << "Sum of squares from 1 to " << n - 1 << ": " << sum << "\n";
	real check_sum = 0;
	t.startTimer();
	for(real i = 1; i < n; i++) {
		check_sum += i * i;
